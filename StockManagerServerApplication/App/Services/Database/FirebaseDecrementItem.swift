...
extension FirebaseWrapper {
    ...
    class func decrementItem(_ itemID: String, value: Int, type: String, storeID: String) -> FirebaseWrapperVoidResult {
        ...
        if type == "backstockQuantity" {
            ...
            FirebaseWrapper.itemReference(userDesignatedID: itemID, storeID: storeID).getDocuments(completion: { (snapshot, err) in
                ...
                if let doc = snapshot?.documents.first {
                    ...
                    // update the document
                    FirebaseWrapper.itemReference(itemUUIDString: uuid, storeID: storeID).updateData(["backstockQuantity" : FieldValue.increment(Double(newValue))]) { (err) in
            ...
            FirebaseWrapper.itemReference(userDesignatedID: itemID, storeID: storeID).getDocuments(completion: { (snapshot, err) in
                ...
                if let doc = snapshot?.documents.first {
                    ...
                    let uuid = doc.documentID
                    // update the document
                    FirebaseWrapper.itemReference(itemUUIDString: uuid, storeID: storeID).updateData(["customerAccessibleQuantity" : FieldValue.increment(Double(newValue))]) { (err) in
                        // if there is an error, set the error
                        if let _ = err {
                            result = false;
                            error = StockManagerError.DatabaseErrors.connectionError
